...
class VOCDataset:
    def __init__(self):
        self.image_dir = os.path.join(DATASET, "JPEGImages")
        self.annotation_dir = os.path.join(DATASET, "Annotations")
        self.ids = [x[:-4] for x in os.listdir(self.image_dir)]
        self.train_ids = self.ids[: len(self.ids) * 3 // 4]
        with open(os.path.join(DATASET, "ImageSets/Main/trainval.txt")) as f:
            self.train_ids = [x.strip() for x in f]
        with open(os.path.join(DATASET, "ImageSets/Main/test.txt")) as f:
